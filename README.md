...
al canal y protegiendo cada mensaje con uno de dos algoritmos de integridad:

- **Hamming genérico `(n, m)`** — corrección de errores (bloques de 8 bits).
...
El emisor pedirá, en orden: el algoritmo de integridad (`hamming` o
`crc32`), la tasa de error por bit (ej. `0.01`), y luego el flujo normal del
...
```bash
python3 pruebas/motor.py        # genera pruebas/resultados/resultados.csv
