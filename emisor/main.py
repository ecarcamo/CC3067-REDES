...
def enviar(sock: socket.socket, texto: str, algoritmo: str, probabilidad: float, configuracion: dict | None = None) -> None:
...
def login(sock: socket.socket, lector: transmision.LectorLineas, algoritmo: str, probabilidad: float, configuracion: dict) -> bool:
    """Pide tarjeta y PIN, reintenta hasta autenticar."""
    while True:
        enviar(sock, aplicacion.solicitar_login(), algoritmo, probabilidad, configuracion)
...
def menu(sock: socket.socket, lector: transmision.LectorLineas, algoritmo: str, probabilidad: float, configuracion: dict) -> None:
    """Muestra el menu y maneja las opciones del usuario."""
    while True:
        opcion = aplicacion.solicitar_opcion_menu()

        if opcion == "1":
            enviar(sock, aplicacion.solicitar_retiro(), algoritmo, probabilidad, configuracion)
        elif opcion == "2":
            enviar(sock, aplicacion.mensaje_logout(), algoritmo, probabilidad, configuracion)
            texto, resultado = recibir(lector)
            aplicacion.mostrar_mensaje(texto, resultado)
            break
        else:
...
def main() -> None:
    algoritmo, probabilidad, configuracion = aplicacion.solicitar_configuracion()
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        if login(sock, lector, algoritmo, probabilidad, configuracion):
            menu(sock, lector, algoritmo, probabilidad, configuracion)
